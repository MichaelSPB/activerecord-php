...
/**
 * @version: $Id: Model.php 561 2018-08-06 05:16:35Z VasilevMV $
 */
...
class Model
{
	...
	/**
	 * Contains model old values as column_name => value
	 *
	 * @var array
	 */
	...
	public function assign_attribute($name, $value)
	{
		...
		if ($value instanceof DateTime) {
			$value->attribute_of($this,$name);
                ...
                // если атрибуту присваивается новое значение и это значение не равно старому значению,
                // то атрибут помещается в хранилище старых значений атрибутов
                if (false === $this->is_new_record() && isset($this->attributes[$name])) {
                    $old_value = $this->attributes[$name];
                    if ($old_value != $value) {
                        $this->old_attributes[$name] = $this->attributes[$name];
                    }
                }
        ...
        /**
         * Возвращает (если есть) старое значение
         * 
         * @param string $name Name of an attribute
	 * @return mixed The value of the attribute
         */
        public function old_value($name)
        {
                if (array_key_exists($name,$this->old_attributes))
                        return $this->old_attributes[$name];
        }
	...
	public function &read_attribute($name)
	{
		...
		if (array_key_exists($name,$this->attributes)) {
			...
			// if ($value instanceof DateTime) {
				// global $app;
				// $value->setTimezone(new \DateTimeZone('Etc/GMT-'.(2 * $app['tz'])));
				// return $value;
			// }
	...
	/**
	 * Returns a copy of the model's old_attributes hash if exists.
	 *
	 * @return array A copy of the model's old attribute data
	 */
	public function old_attributes()
	{
